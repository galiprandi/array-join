...
describe("arrayUnion", () => {
    ...
    test("should handle arrays with different types of elements", () => {
        expect(arrayUnion([1, "a", true], [false, "a", 2])).toEqual([1, "a", true, false, 2]);
    });

    test("should handle arrays containing null and undefined", () => {
        expect(arrayUnion([null, undefined, 1], [undefined, 2, null])).toEqual([null, undefined, 1, 2]);
    });
    ...
    test("should preserve order of first occurrences", () => {
        expect(arrayUnion(["b", "a", "c"], ["c", "d", "e"])).toEqual(["b", "a", "c", "d", "e"]);
    });

    test("should throw an error if inputs are not arrays", () => {
        expect(() => arrayUnion(null as any, [1, 2, 3])).toThrow("Both arguments must be arrays");
        expect(() => arrayUnion([1, 2, 3], undefined as any)).toThrow("Both arguments must be arrays");
    });
});
